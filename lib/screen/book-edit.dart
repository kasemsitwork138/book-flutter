...
class bookEdit extends StatefulWidget {
  ...
  const bookEdit({super.key, required this.book});
...
class _bookEditState extends State<bookEdit> {
  ...
  void _loadBookData() {
    if (widget.book != null) {
      ...
      _publishedDateController.text = widget.book['published_date'] ?? '';
    }
  }
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          ...
          _publishedDateController.text = data['published_date'];
          // _imageBytes = data['cover_image'];
          // _imageName = data['cover_image_name'];
        });
      }
    } catch (e) {
      print(e);
    ...
    try {
      ...
      if (res.statusCode == 201) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('แก้ไขหนังสือไม่สำเร็จ')));
        }
      }
    } catch (e) {
      print(e);
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('แก้ไขหนังสือ', style: TextStyle(fontSize: 28)),
              SizedBox(height: 20),
                ...
                onPressed: () async {
                  ...
                  try {
                    ...
                    var request = http.MultipartRequest(
                      'POST',
                      Uri.parse(
                        'http://localhost:8000/api/books/${widget.book['id']}',
                      ),
                    );
